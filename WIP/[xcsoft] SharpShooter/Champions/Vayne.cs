...
namespace Sharpshooter.Champions
{
    public static class Vayne
    {
        ...
        {
            Q = new Spell(SpellSlot.Q, (float)(300 + Orbwalking.GetRealAutoAttackRange(Player)));
            ...
            SharpShooter.Menu.SubMenu("Drawings").AddItem(new MenuItem("drawingP", "Passive Range", true).SetValue(new Circle(false, Color.FromArgb(183, 0, 0))));
            SharpShooter.Menu.SubMenu("Drawings").AddItem(new MenuItem("drawingQ", "Q Range", true).SetValue(new Circle(false, Color.FromArgb(183, 0, 0))));
        ...
        static void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                foreach (var En in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsEnemy && !hero.IsDead && hero.IsTargetable && hero.IsVisible))
                {
        ...
        static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (E.CanCast(gapcloser.Sender) && gapcloser.Sender.IsTargetable)
                E.Cast(gapcloser.Sender);
            else if (Q.CanCast(gapcloser.Sender) && gapcloser.Sender.IsTargetable)
                Q.Cast(gapcloser.Sender.Position.Extend(Player.Position, 300));
        }
        ...
        static void Interrupter_OnPossibleToInterrupt(Obj_AI_Base unit, InterruptableSpell spell)
        {
            ...
            if (E.CanCast(unit) && unit.IsTargetable)
                E.Cast(unit);
        }

        static void Obj_AI_Hero_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsMe && args.SData.Name == "vayneinquisition" && SharpShooter.Menu.Item("AutoRQ", true).GetValue<Boolean>() && Q.IsReady())
        ...
        static void Combo()
        {
            ...
            {
                foreach (var En in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsEnemy && !hero.IsDead && hero.IsTargetable && hero.IsValidTarget(E.Range)))
                {
                    var EPred = E.GetPrediction(En);
                    int pushDist = 450;
